...
def ranking_q_object(q_init, X, u, lamda, mu, gamma):
    ...
    #  regularization
    obj = obj + (gamma/2) * np.dot(alpha, alpha)  # alpha[i]??
    obj = obj / nc
...
def maxTheta(q, x , u , lamda):
    ...
    for k in range(0, nf):
        ...
        th[k] = (-1 / lamda * nc) * sum([np.dot(q_minus_u[i], temp[i]) for i in range(nc)])
...
def trainAdversarialRanking(x, u, lamda, mu, gamma):
    ...
    optim = optimize.minimize(ranking_q_object, x0 = q_alpha_init, args=(x, u, lamda, mu, gamma), method='L-BFGS-B', jac=True,  bounds=bd)
    print(optim)
