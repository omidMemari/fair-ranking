function Q = trainAdversarial()

addpath('minConf/minFunc');
addpath('minConf/minConf');
addpath('minConf');
addpath('lbfgsb');
addpath('lbfgsb/lbfgsb');
addpath('lbfgsb/lbfgsb_wrapper');
load data
...
%init q
for i = 1:nc
    Q_init(:,i) = rand(1,nn+1); % or (nn+1,1)???
end
...
%[q_opt,f,info] = lbfgsb( funObj, l, u, options );
...
temp = py.adv_train2.adversarialRankingObj() %[f, g] = py.adv_train2.adversarialRankingObj() % pass data using .mat file
